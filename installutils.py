...
def verify_fqdn(host_name, no_host_dns=False, local_hostname=True):
    ...
    if host_name != host_name.lower():
        raise BadHostError(
            "Invalid hostname '%s', must be lower-case." % host_name)
    ...
    for a in hostaddr:
        ...
        if address in ('127.0.0.1', '::1'):
            raise HostForwardLookupError(
                "The IPA Server hostname must not resolve to localhost (%s). A routable IP address must be used. Check /etc/hosts to see if %s is an alias for %s" % (address, host_name, address))
        try:
...
def get_server_ip_address(host_name, unattended, setup_dns, ip_addresses):
    ...
    if hostaddr.intersection(
        ...
        print(
            "Please fix your /etc/hosts file and restart the setup program", file=sys.stderr)
        raise ScriptError()
    ...
    if ip_addresses:
        if setup_dns:
            ...
            if set(ip_addresses) <= set(ips):
                ...
                print(
                    "Error: the hostname resolves to IP address(es) that are different", file=sys.stderr)
                print(
                    "from those provided on the command line.  Please fix your DNS", file=sys.stderr)
    ...
    for ip_address in ips:
        ...
        if hosts_record is not None:
            ...
            if primary_host != host_name:
                ...
                print(hosts_record[0], " ".join(
                    hosts_record[1]), file=sys.stderr)
                print("Chosen hostname %s does not match configured canonical hostname %s"
                      % (host_name, primary_host), file=sys.stderr)
                print(
                    "Please fix your /etc/hosts file and restart the installation.", file=sys.stderr)
                raise ScriptError()
...
def check_available_memory(ca=False):
    ...
    if in_container():
        ...
        ) and os.path.exists('/sys/fs/cgroup/memory.current'):
            with open('/sys/fs/cgroup/memory.max') as fd:
